...
public class CommonChangelogGeneratorTests
{
    ...
    public async Task GenerateAsync_ShouldReturnChangelogWithNoGroupsForEmptyChangeSet()
    {
        ...
        // Assert
        await Assert.That(changelog.Groups).IsNotNull();
        await Assert.That(changelog.Groups).IsEmpty();
    }
    ...
    public async Task GenerateAsync_ShouldReturnChangelogWithGroupsForNonEmptyChangeSet()
    {
        ...
        // Assert
        await Assert.That(changelog.Groups).IsNotNull();
        await Assert.That(changelog.Groups).IsNotEmpty();
        await Assert.That(changelog.Groups.Count()).IsEqualTo(1); // Assuming one group for a single "feat" commit
        await Assert.That(changelog.Groups[0].Title).IsEqualTo("Features");
        await Assert.That(changelog.Groups[0].Items.Count()).IsEqualTo(1);
        await Assert.That(changelog.Groups[0].Items[0].Description).IsEqualTo("subject");
    }
}
